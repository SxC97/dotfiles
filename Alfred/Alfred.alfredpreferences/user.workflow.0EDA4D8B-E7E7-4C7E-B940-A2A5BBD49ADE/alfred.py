...
class AlfredWorkflow(object):
    ...
    def write_items(self, items, delete_none=True):
        if delete_none:
    ...
    def item(self, title, description=None, arg=None, ignore=False, icon=None,
             uid=None, autocomplete=True, match=None):
        if match and arg and len(match.strip()) \
           and not match.lower() in str(arg).lower():
            return None
        options = {}
        if not uid:
            uid = 0
        options[u'uid'] = get_uid(uid)
        if arg:
            options[u'arg'] = arg
            if autocomplete:
                options[u'autocomplete'] = arg
        if ignore:
            options[u'ignore'] = 'yes'
        return Item(options, title, description, icon)
